...
let isFreeplayOnUpdategoing: boolean = false // a variable that holds wheater or not free play is giveing the player points
let speed: any = 120
let fpause: any = 800
let sideasteroids: string = ' Off'
let realSideAsteroids: boolean = false
let isInMenue: boolean = true
let savefreeplay: string = 'On'
let topAsteroids: string = 'On'
let realTopAsteroids: boolean = true
let sideSpeed: number = 92
let fSidePause: number = 1900
let fSideMinSpeed: number = 92
...
let vertionNumber: string = '1.2' // variable to cnage the verstion in only one place compleatly for my convinece while codeing 
let godmodetext: any = null
let astronot: Sprite = null
let fuel: Sprite = null
let fuelbar: any = null
...
function lvl3 (lodrkt: boolean) {
    ...
    timer.background(function() {
        bLvlSetUp(80, 1000, 15, false, lodrkt, 3)
    })

  timer.background(function() {
      advLvlSetUp(90, 100, 1900, 10, false, false, 3, 50, 85, -5, false, true, false)
  })
  pause(300)
...
function lvl4 () {
    function lvl4meat() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            tiles.placeOnRandomTile(fuel, assets.tile`fuelSpawnTile`)

            sprites.onOverlap(SpriteKind.Food, SpriteKind.Food, function(sprite: Sprite, othersprite: Sprite) {
                tiles.placeOnRandomTile(othersprite, assets.tile`fuelSpawnTile`)
            })

        }
        
        color.setColor(6, color.rgb(188, 71, 21))

        // astronot set up
        sprites.destroyAllSpritesOfKind(SpriteKind.Player)
        astronot = sprites.create(assets.image`astronotFront`, SpriteKind.Player)
        scaling.scaleToPercent(astronot, 150, ScaleDirection.Uniformly, ScaleAnchor.Middle)
        scene.cameraFollowSprite(astronot)
        controller.moveSprite(astronot, 100, 0)
        tiles.placeOnTile(astronot, tiles.getTileLocation(1, 14))
        astronot.ay = 500

        amuntOfFuel = 0

        fuelbar = statusbars.create(20, 4, StatusBarKind.Energy)
        fuelbar.attachToSprite(astronot)
        fuelbar.setLabel(`Fuel: ${amuntOfFuel}/5 `)
        fuelbar.setColor(7, 2)
        fuelbar.value = 0

        controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
            if (astronot.isHittingTile(CollisionDirection.Bottom)) {
                astronot.vy = -250
            }
        })
        ...
        statusbars.onStatusReached(StatusBarKind.Energy, statusbars.StatusComparison.EQ, statusbars.ComparisonType.Percentage, 100, function (status) {
            ...
            if (story.getLastAnswer() == 'Play cut scene') {
                astronot.destroy()
                tiles.setCurrentTilemap(tilemap`blank tile map`)
                scene.setBackgroundImage(assets.image`marsBackground`)
                level = 4.5
                Ship = sprites.create(assets.image`Defalt Ship`, SpriteKind.Player)
                Ship.setStayInScreen(false)
                Ship.setPosition(80, 80)
                controller.moveSprite(Ship, 0, 0)
                scene.setBackgroundImage(assets.image`marsBackground`)
                
                astronot = sprites.create(assets.image`astronotFront`, SpriteKind.Player)
                astronot.setPosition(180, 80)

                animation.runImageAnimation(astronot, assets.animation`astrnotLeftWalk`, 80, true)
                story.spriteMoveToLocation(astronot, 85, 80, 50)
                astronot.destroy()
                pause(500)
    ...
    `, img`
        ...
        controller.right.onEvent(ControllerButtonEvent.Pressed, function() {
            animation.runImageAnimation(astronot, assets.animation`astrnotRightWalk`, 80, true)
        }) 

        controller.right.onEvent(ControllerButtonEvent.Released, function() {
        
            animation.stopAnimation(animation.AnimationTypes.All, astronot)
            animation.runImageAnimation(astronot, [assets.image`astronotFront`], 10, true)
        })
        ...
        story.spriteSayText(astronot, 'We need to find all of the fuel sacterd about the planet!')
    ...
    if (story.getLastAnswer() == 'Play cut scene') {
        ...
        astronot = sprites.create(assets.image`astronotFront`,SpriteKind.Player)
        astronot.setPosition(85,80)
        
        pause(500)
        animation.runImageAnimation(astronot, assets.animation`astrnotRightWalk`, 80, true)
        story.spriteMoveToLocation(astronot, 180, 80, 50)
        lvl4meat()

    } else if (story.getLastAnswer() == 'Skip cut scene') {
        lvl4meat()
    }
}
